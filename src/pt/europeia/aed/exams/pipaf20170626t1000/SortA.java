...
//This is insertion sort with swaps:
public final class SortA {
    public static void sort(final double[] values) {
        for (int n = 1; n < values.length; n++)
            for (int i = n; i != 0 && values[i] < values[i - 1]; i--)
                swap(values, i - 1, i);
    }

    private static void swap(final double[] values, final int i,
            final int j) {
